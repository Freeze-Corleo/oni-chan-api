...
class CategoryProductController{
    ...
    public static async requestCreateOne(req: express.Request, res: express.Response){
        return res.send(await CategoryProductController.createOne(req.body));
    }

    public static async requestUpdateById(req: express.Request, res: express.Response){
        return res.send(await CategoryProductController.updateById(String(req.query.id), req.body));
    }
    ...
    private static async createOne(categoryProductWanted: ICategoryProduct){
        try {
            const createdCategoryProduct = await new categoryProduct(categoryProductWanted).save();
            if(!createdCategoryProduct) {
              throw new Error('No document found');
            }
            return JSON.stringify(createdCategoryProduct);
        } catch (error) {
            Log.error(error);
            return JSON.stringify('Cannot create a new category product ');
        }
    }

    private static async updateById(id: string, categoryProductWanted: ICategoryProduct){
        try {
            const updatableCategoryProduct = await categoryProduct.findOneAndUpdate({_id: id}, categoryProductWanted);
